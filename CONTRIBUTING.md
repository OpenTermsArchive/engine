...
6. For non-functional changes (docs, refactors, internal tooling, …) that should not trigger a release, the changelog should be left untouched and the label `No Release` must be added to the pull request to prevent triggering the changelog validation and the release workflows.
