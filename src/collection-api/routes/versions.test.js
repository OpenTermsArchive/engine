...
describe('Versions API', () => {
  ...
  describe('GET /version/:versionId', () => {
    ...
    context('when a referenced snapshot is absent from the repository', () => {
      let versionWithMissingSnapshot;

      before(async () => {
        versionWithMissingSnapshot = new Version({
          serviceId: 'service-missing-snapshot',
          termsType: 'Terms of Service',
          content: 'content referencing a snapshot that is missing from the repository',
          fetchDate: FETCH_DATE,
          snapshotIds: ['ecd9407eb26b1bf0613186175ee80edbdeedd47f'], // well-formed ID that is absent from the snapshots repository
        });
        await versionsRepository.save(versionWithMissingSnapshot);
        response = await request.get(`${basePath}/v1/version/${versionWithMissingSnapshot.id}`);
      });

      it('responds with 200 rather than failing on the unresolved snapshot', () => {
        expect(response.status).to.equal(200);
      });

      it('degrades fetchUrls to an empty array', () => {
        expect(response.body.fetchUrls).to.deep.equal([]);
      });
    });
  });
  ...
  describe('GET /version/:serviceId/:termsType/latest', () => {
    ...
    context('when versions exist', () => {
      ...
      it('returns diff stats (additions and deletions)', () => {
        expect(response.body).to.have.property('additions');
        expect(response.body).to.have.property('deletions');
      });
  ...
  describe('GET /version/:serviceId/:termsType/:date', () => {
    ...
    context('when a version is found', () => {
      ...
      it('returns the expected version', () => {
        expect(response.body).to.deep.include(expectedResult);
      });
