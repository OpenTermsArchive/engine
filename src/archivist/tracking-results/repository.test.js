...
describe('TrackingResultsRepository', () => {
  ...
  describe('when a commit fails', () => {
    const commitError = new Error('Unable to create index.lock');

    context('for a terms that already has a committed result', () => {
      let error;

      before(async () => {
        await subject.saveTermsResult(makeResult());

        const commitStub = sinon.stub(subject.git, 'commit').rejects(commitError);

        error = await subject.saveTermsResult(makeResult({ status: STATUSES.failed, reasons: ['[fetch] HTTP 500'] })).catch(error => error);
        commitStub.restore();
      });

      after(() => subject.removeAll());

      it('throws an error exposing the cause', () => {
        expect(error.cause).to.equal(commitError);
      });

      it('keeps the committed result as the latest one', async () => {
        const latest = await subject.findLatestTermsResult(SERVICE_ID, TERMS_TYPE);

        expect(latest.status).to.equal(STATUSES.ok);
      });
    });

    context('for a terms that has no committed result yet', () => {
      before(async () => {
        const commitStub = sinon.stub(subject.git, 'commit').rejects(commitError);

        await subject.saveTermsResult(makeResult()).catch(() => {});
        commitStub.restore();
      });

      after(() => subject.removeAll());

      it('leaves no result behind', async () => {
    ...
    context('for the run', () => {
      before(async () => {
        ...
        const commitStub = sinon.stub(subject.git, 'commit').rejects(commitError);
        const crashed = makeRun();

        crashed.markCrashed('2026-04-06T11:00:00Z');
        await subject.saveRun(crashed).catch(() => {});
        commitStub.restore();
      });

      after(() => subject.removeAll());

      it('keeps the committed run as the latest one', async () => {
        const latest = await subject.findLatestRun();

        expect(latest.lastRun.status).to.equal('in_progress');
      });
    });
  });
  ...
  describe('#saveRun', () => {
    ...
    context('called twice in a row with identical content', () => {
      ...
      before(async () => {
        await subject.saveRun(makeRun());
        commitsBefore = (await git.log()).length;
        secondSha = await subject.saveRun(makeRun()); // Same content as the previous saveRun
  ...
  describe('#initialize', () => {
    it('is idempotent: calling it twice does not throw and leaves the repo usable', async () => {
      ...
      await subject.initialize();
      await subject.saveTermsResult(makeResult());
      expect(await subject.findLatestTermsResult(SERVICE_ID, TERMS_TYPE)).to.not.be.null;
      await subject.removeAll();
    });
  });
  ...
  describe('#findLatestRunCommitSha', () => {
    ...
    context('after a single saveRun', () => {
      ...
      before(async () => {
        savedSha = await subject.saveRun(makeRun());
      });
    ...
    context('after several saveRun calls', () => {
      ...
      before(async () => {
        ...
        lastSha = await subject.saveRun(completed);
      });
    ...
    context('when the latest activity is on a per-terms file rather than run.json', () => {
      ...
      before(async () => {
        runSha = await subject.saveRun(makeRun());
  ...
  describe('#findCommittedTermsResultsSince', () => {
    ...
    context('when run.json is committed in between', () => {
      ...
      before(async () => {
        ...
        await subject.saveRun(completed);
  ...
  describe('crash recovery scenario', () => { // End-to-end test of the primitives composed the way the recorder composes them
    ...
    before(async () => {
      ...
      await subject.saveRun(startedRun);
      await subject.saveTermsResult(makeResult({ serviceId: 'Facebook', termsType: 'Terms of Service' }));
