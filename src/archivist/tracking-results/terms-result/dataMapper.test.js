...
describe('tracking-result/dataMapper', () => {
  ...
  describe('#determineEventType', () => {
    context('with no previous result', () => {
      ...
      context('when the new result is a failure', () => {
        it('returns FIRST_TRACKING_FAILURE', () => {
          expect(determineEventType(null, makeResult({ status: STATUSES.failed, reasons: ['[fetch] HTTP 500'] }))).to.equal(EVENT_TYPES.FIRST_TRACKING_FAILURE);
        });
      });
    });
    ...
    context('when transient error reasons change while both runs have one', () => {
      it('returns null', () => {
        ...
        expect(determineEventType(prev, next)).to.be.null;
      });
    });
  ...
  describe('#formatMessage', () => {
    ...
    it('formats FIRST_TRACKING_FAILURE as a tracking failure', () => {
      expect(formatMessage(EVENT_TYPES.FIRST_TRACKING_FAILURE, { ...params, failureType: 'fetch' })).to.equal('Record tracking failure of Facebook Terms of Service (fetch)');
    });
  ...
  describe('#toPersistence', () => {
    ...
    context('on first record of a failure', () => {
      it('uses the tracking-failure commit subject', () => {
        const persistence = toPersistence(makeResult({ status: STATUSES.failed, reasons: ['[extraction] no match'] }), null);

        expect(persistence.message).to.equal('Record tracking failure of Facebook Terms of Service (extraction)');
      });
    });
