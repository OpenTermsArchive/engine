...
process.on('uncaughtException', err => {
  if (Object.keys(err).includes('smtp')) {
    console.error('This is an smtp error that we do not know how to handle in winston but do not want to track');
    console.error('We do not use logger.error here because it would potentially retry to send the error by email');
