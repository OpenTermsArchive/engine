...
const WINDOWS_RESERVED_CHARACTERS_REGEXP = /[:"<>|*?]/; // Forbidden in Windows file names, so a segment holding one cannot exist on every supported platform; the service ID guidelines exclude them for the same reason
...
// Accepts only segments that are usable as a file name on every supported platform. As git C-quotes only double-quotes, backslashes and control characters in the paths it prints (see core.quotePath), all rejected here, an accepted segment also never comes back quoted from a git log.
export function isPortableFileName(segment) {
  return isPlainPathSegment(segment) && !WINDOWS_RESERVED_CHARACTERS_REGEXP.test(segment);
}
