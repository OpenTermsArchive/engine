...
(async function main() {
  ...
  const fromSnapshotsRecords = await (await migration.from.snapshots.source.getAll());
  const toSnapshotsRecords = await (await migration.to.snapshots.source.getAll());
  ...
  const fromVersionsRecords = await (await migration.from.versions.source.getAll());
  const toVersionsRecords = await (await migration.to.versions.source.getAll());
